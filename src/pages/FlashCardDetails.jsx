...
import {
  ...
  const handleDownloadImage = () =>{
    ...
    .then(function (dataUrl) {
      ...
      link.download = "demoimageFlashCard.jpeg"
      link.href = dataUrl;
      link.click();
  ...
  return (
    <section className='flex flex-col text-slate-600'> 
      <header className='flex'>
        <BiArrowBack 
        className='text-3xl mr-6 cursor-pointer ml-40 '
        onClick={() => navigate(-1)}
        />
      ...
      <main className="mt-6 grid grid-rows-1 md:grid-cols-4" style={{ color: props.mode === "white" ? "black" : "white"}}>
        <aside className="col-span-1 bg-white w-[70vw] md:w-[14rem] xl:w-[17rem] m-5 ml-16 px-4 py-5 h-fit mr-2 rounded-md " style={{ backgroundColor: props.mode === "white" ? "white" : "rgb(30 41 59)" , color: props.mode === "white" ? "black" : "white"}}>
          <h2 className="p-2 text-lg font-semibold">Flashcards</h2>
          <hr />
          ...
          {ourCard.cards &&
            ourCard.cards.map((card) => (
              <p style={{ color: props.mode === "white" ? "black" : "white"}}
                key={card.cardid}
                className={`py-2 px-8  font-medium hover:bg-slate-100 hover:text-red-500 cursor-pointer  ${
    ...
    className='col-span-3 md:col-span-2 flex flex-col xl:flex-row items-center w-full bg-white shadow-lg rounded-lg '>
      ...
      <p className={`w-full p-6 py-10 font-medium text-lg`}>
        {singleCardDetail.carddescription}
      </p>
      </section>

      <aside className='col-span-1 hidden md:flex flex-col items-center space-y-5 mr-20'>
...
<div className='popupBox' style={{display: share}}>
  <div className='relative w-11/12 xl:w-2/5 sm:w-11/12 p-3 sm:p-8 bg-white rounded-lg inline-table' style={{ backgroundColor: props.mode === "white" ? "white" : "rgb(12 74 110)" , color: props.mode === "white" ? "black" : "white"}}>
    <h3 className='text-lg font-semibold mb-2' style={{color: props.mode === "white" ? "black" : "white"}}>Share :</h3>
    <div className='flex sm:items-center flex-col sm:flex-row ' style={{ backgroundColor: props.mode === "white" ? "white" : "rgb(30 41 59)"}}>
      <span className='w-6/7 text-lg px-2 py-6 rounded-lg outline-dashed outline-1 outline-blue-200 inline-table'>
        <span>Link: </span>&nbsp;&nbsp;
      <span className='inline-block font-medium'>{url}</span>
      ...
      <span className='flex mt-3 sm:mt-0'>
        <BiCopy className='text-2xl ml-4  mb-5 cursor-pointer'
